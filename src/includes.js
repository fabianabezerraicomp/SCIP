...
function includeHTML3() {
    ...
    for (i = 0; i < z.length; i++) {
        elmnt = z[i];
        /*search for elements with a certain atrribute:*/
        file = elmnt.getAttribute("logout-include-html");
        if (file) {
            /* Make an HTTP request using the attribute value as the file name: */
            xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function () {
                if (this.readyState == 4) {
                    ...
                    /* Remove the attribute, and call this function once more: */
                    elmnt.removeAttribute("logout-include-html");
...
window.setTimeout(includeHTML1, 100);
window.setTimeout(includeHTML2, 100);
window.setTimeout(includeHTML3, 100);